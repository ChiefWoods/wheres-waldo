...
const collectSceneCharacterCoordinates = async (
  scenes: SceneSeedInput[],
  characters: string[],
): Promise<Map<string, Map<string, SceneCharacterSeedInput>>> => {
  ...
  for (const fileName of csvFiles) {
    ...
    for (const characterName of characters) {
      if (!sceneCharacters.has(characterName)) {
        throw new Error(
          `Missing coordinates for character ${characterName} in scene ${sceneSlug}.`,
        );
      }
    }
  ...
  for (const scene of scenes) {
    if (sceneCharacterCoordinatesBySceneSlug.has(scene.slug)) {
      continue;
    }

    const placeholderCharacters = new Map<string, SceneCharacterSeedInput>();
    for (const characterName of characters) {
      const { xNorm, yNorm } = placeholderCoordinates(scene.slug, characterName);
      placeholderCharacters.set(characterName, {
        characterName,
        xNorm,
        yNorm,
      });
    ...
    sceneCharacterCoordinatesBySceneSlug.set(scene.slug, placeholderCharacters);
  }

  return sceneCharacterCoordinatesBySceneSlug;
};

export type { SceneCharacterSeedInput, SceneSeedInput };
export { collectCharacters, collectSceneCharacterCoordinates, collectScenes };
