  ...
  for (const fileName of csvFiles) {
    ...
    for (const row of parseResult.data) {
      ...
      sceneCharacters.set(characterName, {
        characterName,
        xNorm: roundTo4(xFromLeftNorm),
        yNorm: roundTo4(yFromTopNorm),
      });
    }
...
const updateDatabase = async (prisma: PrismaClient) => {
  ...
  for (const [sceneSlug, sceneCharacters] of sceneCharacterCoordinatesBySceneSlug) {
    ...
    if (!sceneId) {
      throw new Error(
        `Scene ${sceneSlug} does not exist. Add ${sceneSlug}.jpg under ${IMAGE_SCENES_DIR} first.`,
      );
    }
    ...
    for (const sceneCharacter of sceneCharacters.values()) {
      ...
      if (!characterId) {
        throw new Error(
          `CSV for scene ${sceneSlug} references character ${sceneCharacter.characterName}, but no matching character asset exists.`,
        );
      }
